...
import "./LeaveStudentDetail.css"; // Add your styles here
...
const LeaveStudentDetails: React.FC = () => {
    ...
    const fetchStudentDetails = async () => {
      try {
        const res = await axios.get(
          `http://localhost:4000/api/leavestudentDetails/${studentId}`
        );
        setStudent(res.data);
      } catch (err) {
  ...
  const handleDelete = async () => {
    if (
      ...
      try {
        await axios.post(
          `http://localhost:4000/api/student/paststudent/${student._id}`
        );
...
export default LeaveStudentDetails;
